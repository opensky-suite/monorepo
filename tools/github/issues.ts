...
async function createIssues() {
  ...
  console.log(
    `   Delay: ${delay}ms between issues, ${batchDelay}ms between batches\n`,
  );
  ...
  for (let i = 0; i < issues.length; i += batchSize) {
    ...
    console.log(
      `📦 Batch ${batchNum}/${totalBatches} (${i + startFrom}-${i + startFrom + batch.length - 1})`,
    );
...
function generateIssues(
  ...
  for (const product of products) {
    ...
    for (
      let featureIdx = 0;
      featureIdx < product.features.length;
      featureIdx++
    ) {
      ...
      const priority =
        priorities[
          Math.min(
            featureIdx < 3 ? 0 : featureIdx < 6 ? 1 : featureIdx < 8 ? 2 : 3,
            priorities.length - 1,
          )
        ];
  ...
  try {
    switch (command) {
      ...
      default:
        ...
        console.log(
          "  create [--start N] [--count N] [--batch-size N] [--delay MS]",
        );
        console.log(
          "  list [--state open|closed|all] [--labels label1,label2]",
        );
        console.log("  stats");
        process.exit(1);
    }
